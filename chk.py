...
def classify_result(result_line: str) -> str:
    """Extract the status from a result line in the format 'card|STATUS'."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles /start command: Sends welcome message and inline keyboard."""
    welcome_text = (
        "🤖 **Bot Status:** working ✅\n\n"
...
async def upload_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles TXT file upload with card numbers."""
...
async def main():
    ...
    # Use pytz.UTC to ensure the job queue uses a pytz timezone.
    application = ApplicationBuilder().token(bot_token).timezone(pytz.UTC).build()
    # Disable the job queue to avoid timezone errors if you don't need scheduling.
